...
launch_pmmh <- function(filename, nparticles, niterations1, niterations2){
  ...
  save(result_firststep, file = paste0("Results/", filename, "_long_firststep.RData"))
  ...
  save(result_secondstep, file = paste0("Results/", filename, "_long_secondstep.RData"))
}
...
niterations1 <- 5000
niterations2 <- 50000
nparticles <- c(1024, 2048, 1024, 2048)

foreach(i = 1:4, .combine = c) %dopar% {
  launch_pmmh(filenames[i], nparticles[i], niterations1, niterations2)
  1
}
