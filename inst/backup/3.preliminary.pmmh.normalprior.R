...
Prior2 <- new(Module( "batterymodel2normalprior_module", PACKAGE = "BatteryMCMC")$BatteryModel2NormalPrior)
...
foreach(i = 1:n, .combine = c) %dopar% {
  ...
  save(synthetic_data, cholesky_preliminary, theta_preliminary, results, file = paste0("Results/", filenames[i], "_model1_normalprior_preliminary.RData"))

  modellist2 <- list(model = "model2", prior = "normal")
  ...
  cholesky_preliminary <- try(chol(cov(results$chain[(results$niterations/2):results$niterations,])))
  theta_preliminary <- results$chain[results$niterations,]
  save(synthetic_data, cholesky_preliminary, theta_preliminary, results, file = paste0("Results/", filenames[i], "_model2_normalprior_preliminary.RData"))
  1
}
