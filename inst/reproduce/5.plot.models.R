...
output_file <- "Plots/modelcomparison.pdf"

model1filenames <- paste0("Results/", "u_mag1_size930", "_run", 1:nruns, "_model1_uniformprior", ".RData")
model2filenames <- paste0("Results/", "u_mag1_size930", "_run", 1:nruns, "_model2_uniformprior", ".RData")
...
model1chain.df <- foreach(irun = 1:nruns, .combine = rbind) %do% {
  load(model1filenames[irun])
  chain.df <- data.frame(results$chain, iteration = 1:nrow(results$chain))
  chain.df$run <- irun
  chain.df$model <- "model 1"
  chain.df
}

model2chain.df <- foreach(irun = 1:nruns, .combine = rbind) %do% {
  load(model2filenames[irun])
  chain.df <- data.frame(results$chain, iteration = 1:nrow(results$chain))
  chain.df$run <- irun
  chain.df$model <- "model 2"
  chain.df
}

model1chain.df$X7 <- NA
chain.df <- rbind(model1chain.df, model2chain.df)
...
g <- ggplot(chain.df, aes(x = X7, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[7]) +
  facet_grid(model ~ . )
g
...
g <- ggplot(chain.df, aes(x = X1, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[1]) +
  facet_grid(model ~ . )
pdf(file = "Plots/modelRinf.pdf", width = 8, height = 8)
print(g)
dev.off()

g <- ggplot(chain.df, aes(x = X5, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[5]) +
  facet_grid(model ~ . )
pdf(file = "Plots/modelC2.pdf", width = 8, height = 8)
print(g)
dev.off()

g <- ggplot(chain.df, aes(x = X6, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[6]) +
  facet_grid(model ~ . )
pdf(file = "Plots/modelalpha2.pdf", width = 8, height = 8)
print(g)
dev.off()
