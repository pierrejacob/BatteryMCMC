...
output_file <- "Plots/SNR2comparison.pdf"

model1filenames <- paste0("Results/", "u_mag1_size930", "_run", 1:nruns, "_model1_uniformprior", ".RData")
model2filenames <- paste0("Results/", "u_mag1_size930", "_run", 1:nruns, "_model1_uniformprior_SNR2", ".RData")
...
SNRchain.df <- foreach(irun = 1:nruns, .combine = rbind) %do% {
  ...
  chain.df <- data.frame(results$chain, iteration = 1:nrow(results$chain))
  chain.df$run <- irun
  chain.df$SNR <- "S/O noise ratio 10.0"
  chain.df
}

chain.df <- rbind(stdchain.df, SNRchain.df)
chain.df$SNR <- factor(chain.df$SNR, levels = c("S/O noise ratio 0.1", "S/O noise ratio 10.0"),
                       labels = c("S/O noise ratio 0.1", "S/O noise ratio 10.0"))
...
g <- ggplot(chain.df, aes(x = X1, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[1]) +
  facet_grid(SNR ~ . )
pdf(file = "Plots/SNR2Rinf.pdf", width = 8, height = 8)
print(g)
dev.off()

g <- ggplot(chain.df, aes(x = X5, group = run)) + geom_density(aes(y = ..density..), alpha = 0.1) + xlab(labels_theta[5]) +
  facet_grid(SNR ~ . )
pdf(file = "Plots/SNR2C2.pdf", width = 8, height = 8)
print(g)
dev.off()
