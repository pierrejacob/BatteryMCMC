...
foreach (irun = 1:nruns, .combine = c) %dopar% {
  load(file = paste0("Results/", filename, "_model1_uniformprior_SNR_preliminary.RData"))
  ...
  save(results, file = paste0("Results/", filename, "_run", irun, "_model1_uniformprior_SNR.RData"))
  1
}
