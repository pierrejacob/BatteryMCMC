...
void BatteryModel::set_input(const NumericMatrix & input){
...
void BatteryModel::set_parameters(const List & constants){
...
NumericVector BatteryModel::get_parameters(){
...
void BatteryModel::update_parameters(const NumericVector & parameters){
...
void BatteryModel::precomputation(){
...
void BatteryModel::rinit(NumericMatrix & xparticles){
  RNGScope scope;
  // generates x_0
  std::fill(xparticles.begin(), xparticles.end(), 0);
}

NumericMatrix BatteryModel::robs(NumericMatrix & hstates){
  RNGScope scope;
...
void BatteryModel::rtransition(Tree* tree, NumericMatrix & xparticles, int step){
...
void BatteryModel::init_and_weight(NumericMatrix & xparticles, NumericVector & logweights){
  RNGScope scope;
  std::fill(xparticles.begin(), xparticles.end(), 0);
  int nparticles = xparticles.nrow();
  for (int iparticle = 0; iparticle < nparticles; iparticle++){
...
void BatteryModel::transition_and_weight(Tree* tree, NumericMatrix & xparticles, NumericVector & logweights, int step){
  RNGScope scope;
  int nparticles = xparticles.nrow();
...
NumericVector get_observations(BatteryModel* m) { return m->observations; }
NumericMatrix get_states(BatteryModel* m) { return m->states; }
void set_input(BatteryModel* m, NumericMatrix input) { return m->set_input(input); }
void set_observations(BatteryModel* m, NumericMatrix observations) { return m->set_observations(observations); }
void generate_observations(BatteryModel* m, int datalength) { return m->generate_observations(datalength); }

// // expose BatteryModel
RCPP_MODULE(battery_module) {
  class_<BatteryModel>( "BatteryModel" )
  .constructor()
  .field( "Rinf", &BatteryModel::Rinf)
  .field( "C1", &BatteryModel::C1)
  .field( "R1", &BatteryModel::R1)
  .field( "alpha1", &BatteryModel::alpha1)
  .field( "C2", &BatteryModel::C2)
  .field( "alpha2", &BatteryModel::alpha2)
  .field( "A_0_1", &BatteryModel::A_0_1)
  .field( "A_0_2", &BatteryModel::A_0_2)
  .field( "coeff1", &BatteryModel::coeff1)
  .field( "coeff2", &BatteryModel::coeff2)
  ...
  .method( "set_parameters", &BatteryModel::set_parameters)
  .method( "get_parameters", &BatteryModel::get_parameters)
